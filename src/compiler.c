...
static void errorAt(Token *token, const char *message) {
    ...
    fprintf(stderr, "[clox][line %d] Error", token->line);
...
/**
 * Returns true if the current token has the passed type.
 * @param type
 * @return
 */
...
/**
 * Synchronize on the next statement after an error.
 */
...
static uint8_t makeConstant(Value value) {
    ...
    int constant = (int) addConstant(currentChunk(), value);
...
static uint8_t identifierConstant(Token* name) {
    ...
    // Hidden leak memory here :)

    ObjString* string = copyString(name->start, name->length);
    Value indexValue;

    if (tableGet(&stringConstants, string, &indexValue)) {
        // We have already cached the constant inside the table
        return (uint8_t) AS_NUMBER(indexValue);
    }

    uint8_t index = (uint8_t) makeConstant(OBJ_VAL(copyString(name->start, name->length)));
...
static void varDeclaration() {
    ...
    // A variable with no initial value will be set to nil!
...
bool compile(const char* source, Chunk* chunk) {
    ...
    // Until we not reach the EOF, we parse declaration.
